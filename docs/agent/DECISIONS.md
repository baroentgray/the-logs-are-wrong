...
## D-025 — Networked production converges local and network ingress under one admission owner

For TLAW-083 the owner selected Candidate B. In a networked production HostSession, trusted listen-host/local gameplay evidence and successful TLAW-080 resolved network evidence converge **before** `AuthoritativeAcceptedIntent` construction under one session-scoped shared admission/order owner. That owner is the sole final `ServerReceiveSequence` and gameplay-idempotency authority for the networked session and produces the single `IAlreadyAdmittedHostInputSource` consumed by `Gate2ProductionHostDriver`; there is no dual `GetInput`, post-hoc accepted-batch merge, or re-sequencing of already-created receipts. The D-024/TLAW-082 network admission semantics remain authoritative and must be preserved as the network side of this shared owner rather than duplicated by a second production admission authority. TLAW-072 remains unchanged for its existing local/offline/test role and is not used as an independently sequencing second input source in networked production.

All networked-production gameplay ingress shares one per-exact-tick contiguous `ServerReceiveSequence` domain, starting at Zero, and one session/shift-lifetime `IntentId` disposition authority. Trusted local submission in this networked-production path is normalized into the shared owner without traversing FishNet, while network submission retains the exact TLAW-075→076→078→079→080 authority chain before admission. Network receive ticks remain exactly TLAW-076 host-observed ticks; local networked-production evidence must use the same authoritative session-time basis rather than TLAW-072's execution-driven open-tick sequencing. Transport/RPC/source-local counters are not canonical gameplay ordering.

The TLAW-076 inclusive receive boundary remains authoritative and is not replaced by a half-open timestamp rule: `1000 ms -> tick 0`, `2000 ms -> tick 1`, and evidence whose first authoritative host observation maps to receive tick `T` is eligible to be considered in tick `T` before Stage-3 deadline work. Production therefore requires an explicit TLAW-owned ingress/admission-before-seal phase: eligible local/network evidence is captured and normalized, network evidence is resolved through TLAW-080, the shared owner admits/orders the exact tick, then that tick is materialized/sealed through the single host-input boundary, `HostSession.ExecuteTick` runs, and cadence retirement follows. Unity/FishNet callback or render-frame order may not itself decide deadline eligibility. Host backlog never extends deadlines because receive-tick classification remains based on authoritative session elapsed time, not execution progress.

The shared admission owner is lifecycle-bound to the same authoritative HostSession/shift owner: creation, reset, replacement and disposal must clear its sequence/dedupe/pending/seal state, and old-session ingress cannot enter a fresh session. TLAW-080 remains an actor-resolution seam only; successful resolved evidence may be forwarded or queued exactly once toward shared admission without re-reading connection identity, receive time, wire bytes or actor authority. Ordinary exact materialization yields one `AlreadyAdmittedHostTickInput` and keeps existing active-tools evidence unchanged; wrong-shift, double-seal, disposed or invariant misuse fails closed through the existing production host-owner fault boundary.

This decision does not authorize client-visible admission results/acks/replay, retransmission transport, replication, snapshots/resync/reconnect, prediction, host-loss handling, lobby/roster policy, controls, active-tool gameplay, Stage-2 semantic changes, TLAW-076 remapping, or a second HostSession/tick executor. It records architecture only; production implementation remains separately owner-gated.

Sources: [Issue #186](https://github.com/baroentgray/the-logs-are-wrong/issues/186), [owner Candidate B selection comment 5470372408](https://github.com/baroentgray/the-logs-are-wrong/issues/186#issuecomment-5470372408), [pre-D-025 inclusive-boundary correction 5470418016](https://github.com/baroentgray/the-logs-are-wrong/issues/186#issuecomment-5470418016), [TLAW-076 Issue #174](https://github.com/baroentgray/the-logs-are-wrong/issues/174), [D-024](#d-024--gate-3-network-admission-deduplicates-before-per-tick-receive-sequence-allocation), and [BAR-126](https://linear.app/baronet/issue/BAR-126/tlaw-083-architecture-desk-freeze-production-network-intent).

## D-026 — Gate-3 client intent results use a separate session-scoped disposition ledger and replay the original authoritative outcome

For TLAW-085 the owner selected Candidate A1. For a networked production `HostSession`, one Gate-3 result/disposition owner is lifecycle-bound to the same authoritative shift as the D-025 shared admission owner. It stores only client-result correlation and replay state; D-024 remains the sole session gameplay `IntentId` idempotency/admission authority, D-025 remains the sole production admission/order composition, and `HostSession`/Stage 2 remain the sole gameplay authority.

A per-intent client result exists only after authenticated TLAW-079 ingress has successfully decoded D-023 bytes into a valid envelope with a trustworthy `IntentId`. Codec/malformed input without a valid envelope has no per-intent result. Successful decoded network evidence retains its server-observed origin connection and exact TLAW-076 receive tick before TLAW-080. Actor-resolution `ACTOR_NOT_BOUND` is a correlatable pre-admission rejection; unusable or non-live connection failures with no valid live recipient remain server-local/fail-closed.

Before a new network intent may enter shared admission, result capacity is reserved. The store is bounded, does not silently evict session records, and fails closed under saturation rather than allowing an admitted intent whose result cannot be retained. Session reset/disposal clears result state with the networked-production admission lifecycle. The exact numeric capacity and maximum result-payload byte cap remain implementation evidence to be frozen only by the later bounded implementation formalization.

D-024 admission semantics are unchanged. `SHIFT_MISMATCH`, `RECEIVE_TICK_CLOSED`, and `RECEIVE_SEQUENCE_EXHAUSTED` map to stable client rejections; internal invalid-evidence/disposed-owner outcomes remain owner faults. A D-024 duplicate never consumes sequence or re-enters Stage 2. If the original same-origin network record still exists and remains delivery-authorized, a duplicate replays its exact terminal result or returns its retained `PENDING` state. If the original belongs to another/local origin or cannot be safely correlated, the requester receives only generic `INTENT_ID_ALREADY_USED`; no other client's result is disclosed.

Admission success is not terminal gameplay success. An admitted result remains `PENDING` until the exact authoritative tick successfully returns from `HostSession.ExecuteTick`. Only after that successful return does the result owner project the exact matching `AcceptedIntentStageStep`: an existing typed `RejectionReason` becomes terminal `REJECTED`; unsupported action becomes `UNSUPPORTED_ACTION`; Stage-2 duplicate-ignored becomes `INTENT_ALREADY_PROCESSED`; and a known accepted Stage-2 outcome becomes `APPLIED` with its exact resulting authoritative state version. A `HostSession`/invariant fault never becomes an ordinary client result.

The V1 client-result ABI is separate from the D-023 client-intent V1 ABI. Its logical payload carries only schema version, shift id, intent id, disposition `PENDING|APPLIED|REJECTED`, exact authoritative receive tick, optional authoritative state version, and a stable rejection code for `REJECTED`. Disposition tag zero is reserved/invalid; V1 uses `1=PENDING`, `2=APPLIED`, `3=REJECTED`. It carries no `ServerReceiveSequence`, `EventSequence`, connection id, snapshot, or replicated gameplay state. Identifier encoding follows the existing strict UTF-8 Gate-3 identifier rules. Existing `RejectionReason` names are stable client rejection codes; Gate-3-specific codes include at minimum `RECEIVE_TICK_CLOSED`, `RECEIVE_SEQUENCE_EXHAUSTED`, `UNSUPPORTED_ACTION`, `INTENT_ALREADY_PROCESSED`, `INTENT_ID_ALREADY_USED`, and `RESULT_CAPACITY_EXHAUSTED`. New rejection codes may extend V1 without reinterpreting existing values; changing wire fields or encoding requires a new schema version.

Results use a Reliable server→origin-client carrier separate from the D-023 client→server intent carrier. D-026 does not add timer-based retransmission or a standalone result-query protocol: resubmitting the same `IntentId` is the bounded replay mechanism. Pending retry may receive `PENDING`; terminal retry receives the retained terminal value. Result records survive disconnect for gameplay idempotency, but Gate-3 network delivery/replay authorization ends permanently when the original connection disconnects; a reused or replacement connection does not inherit old result access. Reconnect result migration remains separately gated. Trusted listen-host/local submission may consume the same semantic Stage-2 result server-locally and is never forced through FishNet.

Result delivery ordering does not redefine `ServerReceiveSequence`, `EventSequence`, host-tick order, Stage-2 execution order, or the D-025 ingress/admission-before-seal phase. Authoritative domain-event replication remains a separate Gate-3 boundary.

This decision does not authorize production C#/Unity implementation, a result codec or FishNet result handler, result ACK/retransmission timers, standalone result query, `event_sequence` replication, snapshots/gap-resync/reconnect/late join, prediction/reconciliation, host-loss handling, lobby/roster, controls/active tools, Stage-2 semantic changes, D-023/D-024/D-025 rewrites, Ready, merge, cleanup, or the following Gate-3 increment. Production implementation remains separately owner-gated.

Sources: [Issue #190](https://github.com/baroentgray/the-logs-are-wrong/issues/190), [Architecture Desk candidate packet 5537806916](https://github.com/baroentgray/the-logs-are-wrong/issues/190#issuecomment-5537806916), [owner Candidate A1 selection 5538723913](https://github.com/baroentgray/the-logs-are-wrong/issues/190#issuecomment-5538723913), [BAR-128](https://linear.app/baronet/issue/BAR-128/tlaw-085-architecture-desk-freeze-client-visible-intent), [D-024](#d-024--gate-3-network-admission-deduplicates-before-per-tick-receive-sequence-allocation), and [D-025](#d-025--networked-production-converges-local-and-network-ingress-under-one-admission-owner).
