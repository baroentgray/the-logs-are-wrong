# TLAW_LOG_TEST_01 — Provenance

Asset ID: `TLAW_LOG_TEST_01`

Package status: **APPROVED_READY**

Asset acceptance: **PROTOTYPE_ACCEPTED**

Revision: `R01`

Purpose:
- First Asset Factory v0 experiment.
- Test whether Claude can build a convincing parameterized normal production log directly in Blender without Hyper3D.

Approved visual sources:
- `TLAW_LOG_TEST_01_BARK_R01.png` — owner approved; stored in this GitHub package.
- `TLAW_LOG_TEST_01_REFERENCE_R01.png` — owner approved; stored in this GitHub package.

Intended consumer:
- Claude / Blender

Usage:
- Bark image is the approved visual/material source for the bark treatment.
- Reference image is the approved art target for silhouette, proportion, cut ends, bark scale, and overall TLAW presentation.
- Reference is an art target, not geometry that must be copied literally.

Source:
- Workshop image generation.

Notes:
- Owner approved both `BARK · R01` and `REFERENCE · R01`.
- Both approved PNG files are physically stored in this GitHub folder.
- Factory v0 Claude gate is open for revision `R01`.
- Claude must consume the approved sources from this GitHub package rather than reconstructing them from chat history.
- No extra bark maps, cut-end map, anomaly signs, scars, resin, hollow sections, internal objects, multiple log variants, or Hyper3D model belong to this first experiment.
- Package approval was never approval of the resulting 3D asset. The asset was reviewed separately and is recorded above as `PROTOTYPE_ACCEPTED`; it remains an experimental prototype asset.
- Package status and asset acceptance are separate axes. `APPROVED_READY` describes this visual source package; `PROTOTYPE_ACCEPTED` records that the owner accepted the resulting 3D asset visually in the prototype on 2026-08-18.
- `PROTOTYPE_ACCEPTED` implies no promotion and no production integration. The prototype line was deliberately not converted to the card's dimensions.
